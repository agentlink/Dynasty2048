...
@implementation M2Tile {
    ...
    SKAction *_successSound;
    SKAction *_alertSound;
}
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        _successSound = [SKAction playSoundFileNamed:@"merge.wav" waitForCompletion:NO];
        _alertSound = [SKAction playSoundFileNamed:@"move.wav" waitForCompletion:NO];
    }
    return self;
}
...
- (NSInteger)mergeToTile:(M2Tile *)tile
{
    ...
    if (newLevel > 0) {
        ...
        [self runAction:_successSound];
    } else {
        [self runAction:_alertSound];
    }
    return newLevel;
}
...
- (void)refreshValue
{
    ...
    if (value > GSTATE.maxscore)
        GSTATE.maxscore = value;
    
    int values = [self getScoreValue:value];
